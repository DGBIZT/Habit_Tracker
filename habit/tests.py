...
class HabitViewSetTestCase(APITestCase):
    ...
    def test_create_user_habit(self):
        ...
        response = self.client.post(
            reverse("habit:user-habit-list"), self.base_habit_data, format="json"  # Важно: habit:user-habit-list
        )
    ...
    def test_create_pleasant_habit(self):
        ...
        pleasant_habit_data = {**self.base_habit_data, "is_pleasant": True, "linked_habit": None}

        # Удаляем поле reward полностью
        del pleasant_habit_data["reward"]

        # Важно: используем правильное имя URL
        response = self.client.post(reverse("habit:user-habit-list"), pleasant_habit_data, format="json")
    ...
    def test_create_pleasant_with_link_fails(self):
        ...
        base_habit_response = self.client.post(reverse("habit:user-habit-list"), self.base_habit_data, format="json")
        base_id = base_habit_response.data["id"]

        # Пытаемся создать приятную привычку со ссылкой
        pleasant_with_link_data = {**self.base_habit_data, "is_pleasant": True, "linked_habit": base_id}

        response = self.client.post(reverse("habit:user-habit-list"), pleasant_with_link_data, format="json")

        # Проверяем результат
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("non_field_errors", response.data)
        self.assertEqual(
            response.data["non_field_errors"][0], "Приятная привычка не может быть связана с другой привычкой"
        )

    # Тест на обновление привычки
    def test_update_habit(self):
        # Создаем привычку
        response = self.client.post(reverse("habit:user-habit-list"), self.base_habit_data, format="json")
        habit_id = response.data["id"]

        # Обновляем данные
        updated_data = {**self.base_habit_data, "action": "Обновленное действие"}

        # Отправляем PUT запрос
        response = self.client.put(
            reverse("habit:user-habit-detail", kwargs={"pk": habit_id}), updated_data, format="json"
        )
    ...
    def test_delete_habit(self):
        ...
        # Создаем привычку
        response = self.client.post(
            reverse("habit:user-habit-list"), self.base_habit_data, format="json"  # Используем base_habit_data
        )
        ...
        # Удаляем привычку
        delete_response = self.client.delete(reverse("habit:user-habit-detail", kwargs={"pk": habit_id}))
    ...
    def test_list_habits(self):
        ...
        for i in range(3):
            self.client.post(reverse("habit:user-habit-list"), self.base_habit_data, format="json")

        # Получаем список
        response = self.client.get(reverse("habit:user-habit-list"))
    ...
    def test_public_habits(self):
        ...
        public_habit_data = {**self.base_habit_data, "is_public": True}
        response = self.client.post(reverse("habit:user-habit-list"), public_habit_data, format="json")
        habit_id = response.data["id"]

        # Получаем через публичный endpoint
        response = self.client.get(reverse("habit:public-habit-detail", kwargs={"pk": habit_id}))
    ...
    def test_public_habits_permissions(self):
        ...
        private_habit_data = {**self.base_habit_data, "is_public": False}
        response = self.client.post(reverse("habit:user-habit-list"), private_habit_data, format="json")
        habit_id = response.data["id"]
    ...
    def test_permission_denied_for_other_user(self):
        ...
        other_user = get_user_model().objects.create_user(
            username="otheruser", password="123456", email="other@example.com"
        )
        ...
        response = self.client.get(reverse("habit:user-habit-detail", kwargs={"pk": habit_id}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        # Пытаемся обновить привычку
        response = self.client.put(
            reverse("habit:user-habit-detail", kwargs={"pk": habit_id}), self.base_habit_data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        # Пытаемся удалить привычку
        response = self.client.delete(reverse("habit:user-habit-detail", kwargs={"pk": habit_id}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    # Тест на валидацию периодичности
    def test_periodicity_validation(self):
        # Проверяем минимальную периодичность
        invalid_periodicity_data = {**self.base_habit_data, "periodicity": 0}
        response = self.client.post(reverse("habit:user-habit-list"), invalid_periodicity_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Проверяем наличие ошибки в non_field_errors
        self.assertIn("non_field_errors", response.data)
        self.assertIn("Периодичность должна быть положительным числом", str(response.data["non_field_errors"]))

        # Проверяем максимальную периодичность
        invalid_periodicity_data = {**self.base_habit_data, "periodicity": 8}
        response = self.client.post(reverse("habit:user-habit-list"), invalid_periodicity_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Проверяем наличие ошибки в non_field_errors
        self.assertIn("non_field_errors", response.data)
        self.assertIn("Периодичность не может превышать 7 дней", str(response.data["non_field_errors"]))
    ...
    def test_execution_time_validation(self):
        ...
        invalid_time_data_min = {**base_habit_data, "execution_time": 0}
        response = self.client.post(reverse("habit:user-habit-list"), invalid_time_data_min, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Проверяем ошибку для минимального значения
        if "execution_time" in response.data:
            self.assertIn("Время выполнения должно быть положительным числом", str(response.data["execution_time"]))
        elif "non_field_errors" in response.data:
            self.assertIn("Время выполнения должно быть положительным числом", str(response.data["non_field_errors"]))
        else:
            self.fail("Ошибка валидации не найдена в ожидаемых местах")

        # Проверяем максимальное время (3 минуты)
        invalid_time_data_max = {**base_habit_data, "execution_time": 3}
        response = self.client.post(reverse("habit:user-habit-list"), invalid_time_data_max, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Проверяем ошибку для максимального значения
        if "execution_time" in response.data:
            self.assertIn("Время выполнения не должно превышать 2 минуты", str(response.data["execution_time"]))
        elif "non_field_errors" in response.data:
            self.assertIn("Время выполнения не должно превышать 2 минуты", str(response.data["non_field_errors"]))
        else:
            self.fail("Ошибка валидации не найдена в ожидаемых местах")

        # Проверяем корректное время (2 минуты)
        valid_time_data = {**base_habit_data, "execution_time": 2}
        response = self.client.post(reverse("habit:user-habit-list"), valid_time_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["execution_time"], 2)
    ...
    def test_reward_validation(self):
        ...
        response = self.client.post(reverse("habit:user-habit-list"), pleasant_with_reward_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("non_field_errors", response.data)

        # Проверяем награду без связанной привычки
        valid_reward_data = {**self.base_habit_data, "is_pleasant": False, "reward": "Награда", "linked_habit": None}
        response = self.client.post(reverse("habit:user-habit-list"), valid_reward_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
